...
export default function SessionsPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const data = await apiFetch('/presets/sim15', { token });
                const preset = data?.preset;
                const total = Array.isArray(preset?.sessionNames) ? preset.sessionNames.length : 0;
    ...
    useEffect(() => {
        ...
        function onKeyDown(e) { if (e.key === 'Escape') setCreateOpen(false); }
        window.addEventListener('keydown', onKeyDown);
        return () => { clearTimeout(t); window.removeEventListener('keydown', onKeyDown); };
    }, [createOpen]);

    useEffect(() => {
        if (!authOpen) return;
        function onKeyDown(e) { if (e.key === 'Escape') setAuthOpen(false); }
        window.addEventListener('keydown', onKeyDown);
    ...
    function writeCachedPairing(sessionName, payload) {
        try { localStorage.setItem(`pairing:${sessionName}`, JSON.stringify(payload)); } catch { }
    }

    async function startWahaSession(sessionName) {
        await apiFetch(`/waha/sessions/${encodeURIComponent(sessionName)}/start`, { token, method: 'POST' });
    }

    async function fetchQr(sessionName) {
        return await apiFetch(`/waha/sessions/${encodeURIComponent(sessionName)}/qr`, { token, method: 'GET' });
    }
    ...
    async function openAuthForSession(sessionName) {
        ...
        try {
            const next = await loadWahaStatus(token);
            if (next?.[sessionName]?.connected) { setAuthOpen(false); return; }
        } catch { }
        ...
        setAuthBusy(true);
        try {
            try { await startWahaSession(sessionName); } catch (e) { setAuthError(`Start session gagal: ${e?.message || 'Unknown error'}`); return; }
            ...
            if (lastErr && !qr) setAuthError(`Ambil QR gagal: ${lastErr?.message || 'Unknown error'}`);
        } finally {
            setAuthBusy(false);
        }
    }
    ...
    async function requestPairing() {
        ...
        setAuthError('');
        try {
            const data = await apiFetch(`/waha/sessions/${encodeURIComponent(authSessionName)}/pairing-code`, { token, method: 'POST', body: { phoneNumber } });
            const code = data.code || '';
            setPairingCode(code);
            if (code) writeCachedPairing(authSessionName, { code, phoneNumber, createdAt: new Date().toISOString() });
        } catch (e) {
    ...
    async function initSim15Preset() {
        setInitBusy(true);
        setError('');
        try {
            await apiFetch('/presets/sim15/init', { token, method: 'POST' });
            await loadSessions(token);
        ...
        <div className="space-y-6">
            <div className="rounded-2xl border bg-white p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">Kelola 15 nomor peer-to-peer untuk simulasi percakapan.</p>
                    </div>
                    <div className="flex flex-col items-start gap-3 sm:items-end">
                        <button
                            disabled={initBusy}
                            onClick={initSim15Preset}
                        ...
                        >
                            {initBusy ? 'Menyiapkan...' : 'Init SIM15 (15 nomor)'}
                        </button>
            ...
            <section className="rounded-2xl border bg-white p-5">
                <div className="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">15 nomor peer-to-peer. Pairing semua nomor lalu mulai simulasi.</p>
                    </div>
                    <button
                        onClick={async () => { await loadSessions(token); await loadWahaStatus(token); }}
                ...
                <div className="mt-4 space-y-3">
                        ...
                        <div key={s.id} className="rounded-xl border p-4">
                            ...
                            <div className="mt-4 grid grid-cols-1 gap-3">
                                ...
                                <div className="rounded-xl border bg-gray-50 px-4 py-3 text-sm text-gray-700">
                                    Mode auto-reply memakai preset (script). Pair {maxSessions} session lalu mulai simulasi dari tab Simulation.
                                </div>
                            </div>
                        </div>
                    ))}
                    ...
                    {sessions.length === 0 && !loading ? (
                        <div className="rounded-xl border bg-gray-50 px-4 py-3 text-sm text-gray-700">Belum ada session. Klik "Init SIM15" untuk membuat 15 session otomatis.</div>
                    ) : null}
                </div>
            ...
            {createOpen ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <button type="button" className="absolute inset-0 bg-black/50" aria-label="Tutup" onClick={() => setCreateOpen(false)} />
                    <div role="dialog" aria-modal="true" className="relative w-full max-w-lg rounded-2xl bg-white shadow-lg" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between border-b px-6 py-4">
                            <div className="text-lg font-semibold">Buat Sesi Baru</div>
                            <button type="button" className="rounded-lg px-2 py-1 text-gray-500 hover:bg-gray-50" onClick={() => setCreateOpen(false)} aria-label="Close">×</button>
                        </div>
                        <form
                            className="space-y-4 px-6 py-5"
                            onSubmit={(e) => { e.preventDefault(); if (!newSessionName.trim()) return; if (!canCreateMore) return; onCreateSession(); }}
                        >
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Contoh: sim-16"
                                />
                            </div>
                            <div className="flex items-center justify-end gap-2 border-t pt-4">
                                <button type="button" className="rounded-lg border px-4 py-2 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setCreateOpen(false)}>Tutup</button>
                                <button type="submit" disabled={!newSessionName.trim() || !canCreateMore} className="rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-60">Buat Session</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {authOpen ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <button type="button" className="absolute inset-0 bg-black/50" aria-label="Tutup" onClick={() => setAuthOpen(false)} />
                    <div role="dialog" aria-modal="true" className="relative w-full max-w-2xl rounded-2xl bg-white shadow-lg" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between border-b px-6 py-4">
                            <div>
                            ...
                            <button type="button" className="rounded-lg px-2 py-1 text-gray-500 hover:bg-gray-50" onClick={() => setAuthOpen(false)} aria-label="Close">×</button>
                        </div>
                        <div className="grid grid-cols-1 gap-5 px-6 py-5 md:grid-cols-[300px_1fr]">
                            <div>
                                ...
                                <div className="mt-2 rounded-xl border bg-gray-50 p-3">
                                    {qr?.data && qr?.mimetype ? (
                                        <img alt="WAHA QR" src={`data:${qr.mimetype};base64,${qr.data}`} className="h-[260px] w-[260px] rounded-lg border bg-white" />
                                    ) : (
                                ...
                                <button disabled={authBusy || !authSessionName} onClick={refreshQr} className="mt-3 w-full rounded-lg border px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-60">Refresh QR</button>
                            </div>
                            <div className="space-y-3">
                                {authError ? <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{authError}</div> : null}
                                <div className="text-sm text-gray-700">Nomor Telepon (contoh: 6281234567890)</div>
                                <input value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-gray-900/20" placeholder="Contoh: 6281234567890" />
                                <button disabled={authBusy || !authSessionName || !phoneNumber.trim() || !!pairingCode} onClick={requestPairing} className="w-full rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-60">Minta Kode Pairing</button>
                                <div className="text-xs text-gray-500">Format: Awali dengan kode negara tanpa tanda '+', misal Indonesia 62, lalu nomor.</div>
